...
from src.routers.queue.utils import create_queue, get_queue, delete_queue, update_queue, search_by_id_or_extension
...
@queue_router.get("/{queue_id_or_extension_name}", dependencies=[Depends(jwt_validator)], response_model=APIResponse)
async def get_queue_by_id_or_extension(queue_id_or_extension_name: str, token: Annotated[str, Depends(oauth2_scheme)]):
    payload = jwt.decode(token, JWT_SECRET, algorithms=[JWT_ALGORITHM])
    result = await search_by_id_or_extension(queue_id_or_extension_name, payload)
    return result
